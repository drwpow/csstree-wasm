...
describe("tokenize/stream", () => {
  ...
  it("skip()", () => {
    ...
    const targetTokens = tokens.filter(
      (token) => token.type === "ident-token" || token.type === "delim-token",
    );
    const actual = targetTokens
      .map(function (token, idx, idents) {
        return idx
          ? tokens.indexOf(token) - tokens.indexOf(idents[idx - 1])
    ...
    assert.deepStrictEqual(
      actual,
      targetTokens.map((token) => token.type),
    );
  });
