...
describe("Lexer#matchProperty()", () => {
  describe("vendor prefixes and hacks", () => {
    it("vendor prefix", () => {
      const match = lazy.customSyntax.lexer.matchProperty(
        "-vendor-foo",
        lazy.bar,
      );
  ...
  it("should match css wide keywords", function () {
    for (const keyword of cssWideKeywords.concat("test")) {
      const match = lazy.customCssWideKeywords.lexer.matchProperty(
        "color",
        keyword,
      );
  ...
  it("should not be matched to empty value", () => {
    const match = lexer.matchProperty(
      "color",
    ...
    switch (testType) {
      case "valid":
        (it[testState] || it)(name, () => {
          ...
          if (match.error) {
            if (
              /Matching for a tree with var\(\) is not supported/.test(
                match.error.message,
              ) ||
              /Lexer matching doesn't applicable for custom properties/.test(
                match.error.message,
              )
            ) {
              assert(true);
