...
import { lexer } from "../pkg/csstree_wasm.js";
    ...
    if (test.valid) {
      test.valid.forEach((input) => {
        it(`should MATCH to "${input}"`, () => {
          ...
          expect(
            m.match?.map((x) => x.token).filter((x) => x !== undefined),
          ).toEqual(m.tokens.map((x) => x.value).filter((s) => /\S/.test(s)));
        });
      });
    }
    ...
    if (test.matchResult) {
      Object.keys(test.matchResult).forEach((input) => {
        ...
        it(`match result for "${input}"`, () => {
          ...
          expect(processMatchResult(m.match.match)).toEqual(matchResult);
        });
      });
    }
  });
...
describe("syntax matching", () => {
  ...
  it("should raise an error on broken type reference", () => {
    ...
    expect(() => matchAsList(prepareTokens("foo"), matchTree, {})).toThrowError(
