...
const values = lazyValues({
  animationName: () =>
    parse("animation-name", { context: "atrulePrelude", atrule: "keyframes" }),
...
describe("Lexer#matchAtrulePrelude()", () => {
  ...
  it("should not be matched to empty value", () => {
    const match = lexer.matchAtrulePrelude(
      "keyframes",
      parse("", { context: "atrulePrelude", positions: true }),
    );
  ...
  describe("should not match css wide keywords", function () {
    for (const keyword of cssWideKeywords) {
      it(keyword, () => {
        const match = lexer.matchAtrulePrelude(
          "import",
          parse(keyword, { context: "atrulePrelude", positions: true }),
        );
  ...
  describe("should not be matched to at-rules with no prelude", () => {
    ...
    it("with verdor prefix", () => {
      ...
      assert.strictEqual(
        match.error.message,
        "At-rule `@-prefix-font-face` should not contain a prelude",
      );
    });
  });
  ...
  forEachAtrulePreludeTest(
    (testType, testState, name, lexer, atruleName, value) => {
      switch (testType) {
        case "valid":
        ...
        case "invalid":
          (it[testState] || it)(name, () => {
            ...
            assert.strictEqual(
              match.matched,
              null,
              'should NOT MATCH to "' + value + '"',
            );
            assert.strictEqual(
              allowedErrors.includes(match.error.name),
              true,
              "should be one of " + JSON.stringify(allowedErrors),
            );
          });
          break;
      }
