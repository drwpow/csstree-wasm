...
describe("Lexer#matchAtruleDescriptor()", () => {
  it("should match", () => {
    const match = values.customSyntax.lexer.matchAtruleDescriptor(
      "font-face",
      "font-display",
      values.swapValue,
    );
  ...
  describe("vendor prefixes", () => {
    ...
    it("case insensetive with vendor prefix", () => {
      ...
      match = values.customSyntax.lexer.matchAtruleDescriptor(
        "FONT-FACE",
        "FONT-DISPLAY",
        values.swapValue,
      );
  ...
  it("should not be matched to empty value", () => {
    const match = lexer.matchAtruleDescriptor(
      "font-face",
      "font-display",
      parse("", { context: "value", positions: true }),
    );
  ...
  it("should not be matched to at-rules with no descriptors", () => {
    ...
    assert.strictEqual(
      match.error.message,
      "At-rule `@keyframes` has no known descriptors",
    );
  });
  ...
  forEachAtruleDescriptorTest(
    (testType, testState, name, lexer, atruleName, descriptorName, value) => {
      switch (testType) {
        case "valid":
