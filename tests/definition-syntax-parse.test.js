...
describe("definitionSyntax.parse()", () => {
  ...
  describe("bad syntax", () => {
    it("expected a quote", () =>
      assert.throws(() => parse("'x"), /^SyntaxError: Expect an apostrophe\n/));
    ...
    describe("expected a number", () => {
      ...
      for (const test of tests) {
        it(test, () =>
          assert.throws(() => parse(test), /^SyntaxError: Expect a number\n/),
        );
      }
    });
    ...
    describe("missed keyword", () => {
      ...
      for (const test of tests) {
        it(test, () =>
          assert.throws(() => parse(test), /^SyntaxError: Expect a keyword\n/),
        );
      }
    });
    ...
    describe("unexpected input", () => {
      ...
      for (const test of tests) {
        it(test, () =>
          assert.throws(() => parse(test), /^SyntaxError: Unexpected input\n/),
        );
      }
    });
    ...
    describe("bad syntax", () => {
      ...
      for (const test of tests) {
        it(test, () =>
          assert.throws(() => parse(test), /^SyntaxError: Expect `.`\n/),
        );
      }
    });
  });
    ...
    for (const [name, definition] of Object.entries(lexer.atrules)) {
      describe(`atrules/${name}`, () => {
        if (definition.prelude !== null) {
          it("prelude", () =>
            assert.strictEqual(definition.prelude.syntax.type, "Group"));
        }
        ...
        if (definition.descriptors) {
          describe("definitions", () => {
            for (const name in definition.descriptors) {
              it(name, () =>
                assert.strictEqual(
                  definition.descriptors[name].syntax.type,
                  "Group",
                ),
              );
            }
