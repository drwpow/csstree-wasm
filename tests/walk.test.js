...
const notInsideAtrulePrelude = (stack) =>
  stack.every((node) => node.type !== "AtrulePrelude");
...
function createWalkTest(name, test, context, walker, enter, leave) {
  (test.skip ? it.skip : it)(name, () => {
    ...
    assert.deepStrictEqual(actual, expectedWalk(test.ast, enter, leave));
  });
}
...
describe("AST traversal", () => {
  it("base test", () => {
    const ast = parse(
      '@import url("test");@media (min-width: 200px) { .foo:nth-child(2n) { color: rgb(100%, 10%, 0%); width: calc(3px + 5%); content: "test" } }',
    );
  ...
  describe("traverse order", () => {
    const values = lazyValues({
      ast: () =>
        parse(
          ".a.b { foo: bar; baz: qux } .c {} @media all { .d:not(.e > .f) { aa: bb; cc: dd ee } f { ff: gg } }",
        ),
    });
    ...
    it("natural", () => {
      const visitedNames = [];
      const expected =
        "a b foo bar baz qux c media all d not e > f aa bb cc dd ee f ff gg".split(
          " ",
        );
      ...
      assert.deepStrictEqual(visitedNames, expected);
    });

    it("reverse", () => {
      const visitedNames = [];
      const expected =
        "media ff gg f cc ee dd aa bb not f > e d all c baz qux foo bar b a".split(
          " ",
        );
  ...
  describe("break traverse", () => {
    ...
    const nodeName = (node) =>
      node.type +
      (node.name || node.property ? ":" + (node.name || node.property) : "");
  ...
  describe("skip node traverse", () => {
    const ast = parse(
      ".a.b { foo: bar } @media all { selector { foo: bar } } .c.d { foo: bar }",
    );
    const nodeName = (node) =>
      node.type +
      (node.name || node.property ? ":" + (node.name || node.property) : "");
