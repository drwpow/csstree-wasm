...
describe("exports / entry points", () => {
  ...
  it("parser", () => {
    const ast = parse(css);
    assert.strictEqual(
      stringifyWithNoInfo(ast),
      stringifyWithNoInfo(expectedAst),
    );
  });
  ...
  it("convertor", () => {
    ...
    assert.strictEqual(
      findFirstAtrule(ast).prelude.children instanceof utils.List,
      true,
    );
    ...
    assert.strictEqual(
      Array.isArray(findFirstAtrule(ast).prelude.children),
      true,
    );
  ...
  it("lexer", () => {
    ...
    assert.deepStrictEqual(Object.keys(lexer).sort(), ["Lexer"]);
  });
