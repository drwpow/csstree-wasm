...
describe("lexer", () => {
  ...
  it("should not add new unit groups or discard existing", function () {
    ...
    assert.deepStrictEqual(
      Object.keys(customLexer.units),
      Object.keys(lexer.units),
    );
  });
  ...
  it("should allow to override CSS wide keywords", () => {
    ...
    assert.notStrictEqual(
      customLexer.matchProperty("test", "foo").matched,
      null,
    );
    assert.notStrictEqual(
      customLexer.matchProperty("test", "bar").matched,
      null,
    );
    assert.strictEqual(
      customLexer.matchProperty("test", "inherit").matched,
      null,
    );
  });
  ...
  describe("should allow append definitions", function () {
    ...
    it("properties", () => {
      assert.notStrictEqual(
        customSyntax.lexer.matchProperty("color", "foo").matched,
        null,
      );
      assert.notStrictEqual(
        customSyntax.lexer.matchProperty("new", "foo").matched,
        null,
      );
    });
    it("types", () => {
      assert.notStrictEqual(
        customSyntax.lexer.matchType("box", "foo").matched,
        null,
      );
      assert.notStrictEqual(
        customSyntax.lexer.matchType("new", "foo").matched,
        null,
      );
    });
    it("should not append to generic", () => {
      assert.strictEqual(
        customSyntax.lexer.matchType("length", "foo").matched,
        null,
      );
    });
  });
  ...
  describe("dump & recovery", () => {
    ...
    it("custom syntax should match own grammar only", () => {
      assert.notStrictEqual(
        customLexer.matchProperty("test", "1 2aa 3").matched,
        null,
      );
      assert.notStrictEqual(
        customLexer.matchProperty("test", "wide").matched,
        null,
      );
      assert.strictEqual(
        customLexer.matchProperty("color", "red").matched,
        null,
      );
    });
    ...
    it("recovery syntax from dump", () => {
      ...
      assert.notStrictEqual(
        recoverySyntax.lexer.matchProperty("test", "1 2aa 3").matched,
        null,
      );
      assert.notStrictEqual(
        recoverySyntax.lexer.matchProperty("test", "wide").matched,
        null,
      );
    });
  });
