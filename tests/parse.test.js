...
const genericTypesFixture = JSON.parse(
  fs.readFileSync("./fixtures/definition-syntax-match/generic.json"),
);
const stringifyWithNoLoc = (ast) =>
  ...
  describe("basic", () => {
    forEachAstTest((name, test) => {
      (test.skip ? it.skip : it)(name, () => {
        ...
        // AST should be equal
        assert.strictEqual(
          stringifyWithNoLoc(ast),
          stringifyWithNoLoc(test.ast),
        );
      });
    });
    ...
    describe("AnPlusB", () => {
      ...
      fixture.valid.forEach((value) => {
        it(value, () => {
          const actual = parse(":nth-child(" + value + ")", {
            context: "selector",
          }).children.first.children.first.nth;
          ...
          // AST should be equal
          assert.strictEqual(
            stringifyWithNoLoc(actual),
            stringifyWithNoLoc(expected),
          );
        });
      });
  ...
  it("should call onParseError when handler is passed", () => {
    ...
    const ast = parse("{a: 1!; foo; b: 2}", {
      ...
      onParseError: (error, fallbackNode) =>
        errors.push({ error, fallbackNode }),
    });
    ...
    it("formattedMessage", () => {
      assert.throws(
        ...
        (e) => {
          ...
          assert.strictEqual(e.sourceFragment(), "    2 |.\n" + "--------^");
          assert.strictEqual(
            e.sourceFragment(3),
            "    1 |/**/\n" + "    2 |.\n" + "--------^\n" + "    3 |foo",
          );
    ...
    it("as function with API", () => {
      ...
      assert.deepStrictEqual(
        ...
        expected.map((token, index) => ({
          ...
          rangeLocation: offsetToLocation.getLocationRange(
            token.start,
            token.end,
            filename,
          ),
