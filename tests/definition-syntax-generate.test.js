...
const assertGenerateParseRoundTrip = (syntax) =>
  assert.deepStrictEqual(parse(generate(syntax)), syntax);
...
describe("definitionSyntax.generate()", () => {
          ...
          if (definition.descriptors) {
            describe("definitions", () => {
              for (const name in definition.descriptors) {
                it(name, () =>
                  assertGenerateParseRoundTrip(
                    definition.descriptors[name].syntax,
                  ),
                );
              }
            });
  ...
  describe("using decorate", () => {
    ...
    it("all the node types", () => {
      ...
      const expected =
        "{Group}{Group}{Type}<foo>{/Type} && {Property}<'bar'>{/Property}{/Group} | {Group}[ {Token}({/Token} {Keyword}a{/Keyword}{Multiplier}+{/Multiplier} {Comma},{/Comma} {Function}b({/Function} {String}'c'{/String} {Token}/{/Token} {Type}<d>{/Type}{Multiplier}#{1,2}{/Multiplier} {Token}){/Token} {Token}){/Token} ]!{/Group}{/Group}";
      const actual = generate(ast, (str, node) => {
