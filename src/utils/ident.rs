const REVERSE_SOLIDUS: i32 = 0x005c; // U+005C REVERSE SOLIDUS (\)
    ...
    for mut i in 0..str.len() {
        ...
        if (code == REVERSE_SOLIDUS) {
            ...
            if (isValidEscape(REVERSE_SOLIDUS, code)) {
                ...
                let escape_end = consume_escaped(str, escape_start);

                i = escapeEnd - 1;
                decoded += decode_escaped(str.substring(escapeStart + 1, escapeEnd));
            } else {
                // \r\n
...
pub fn encode(str: String) -> String {
    ...
    if (str.len() == 1 && str.as_bytes()[0] == 0x002D) {
        return "\\-";
    }
    ...
    for mut i in 0..str.len() {
        ...
        if (code == 0x0000) {
            encoded.push_str(U + FFFD.to_string());
            continue;
        }
